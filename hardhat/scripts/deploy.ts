...
async function main() {
  ...
  const CalltieFactory = await ethers.getContractFactory("CalltieFactory");
  const contract = await CalltieFactory.deploy();
  await contract.deployed();
  saveFrontendFiles(contract, "CalltieFactory");
  console.log("CalltieFactory deployed to:", contract.address);
